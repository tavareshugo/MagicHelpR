...
setMethod("inferMagicFounder", "MagicGen", 
	...
	n_magic <- apply(gen_prob, 1, sum)
	cat(sum(n_magic == 1), "MAGIC lines were attributed to exactly one founder.\n")
	cat(sum(n_magic == 0), "MAGIC lines were not attributed a founder.\n")
	cat(sum(n_magic > 1), "MAGIC lines were attributed more than one founder.\n")
