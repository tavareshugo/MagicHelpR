...
If we want to see SNP genotypes for marker "MN1_29291":

```{r}
head(getSnpGenotypes(magic_geno)$MN1_29291)
```

If we want to see the genotype probability matrix for that marker:

```{r}
head(getProbGenotypes(magic_geno)$MN1_29291)
```
...
bolt_qtl <- magicQtlScan(magic_phen, "days.to.bolt")
```
...
```{r}
flower_qtl <- magicQtlScan(magic_phen, "bolt.to.flower", cores = 2)
...
```{r}
bolt_cov_qtl <- magicQtlScan(magic_phen, "days.to.bolt",
...
bolt_snp_qtl <- magicQtlScan(magic_phen, "days.to.bolt",
                             marker_cov = peak5)
...
genotypes <- getProbGenotypes(magic_phen)
...
on repeating this estimate several times (500 by default), and so we can 
plot a summary of the estimate like so:

```{r}
ggplot(peak5_effect, aes(accession, mean_phenotype)) +
  geom_pointrange(stat = "summary", fun.data = "median_hilow")
```

We can see from this example that, at this locus:

* The _Zu_ allele should result in later-flowering (~35 days)
* The _Ler_ allele should result in early-flowering (~12 days)
...
ggplot(peak5_effect, aes(accession, mean_phenotype)) +
  ...
  geom_text(data = getFounderGenotypes(magic_phen)[[peak5]], 
            aes(y = 20, label = allele, colour = allele)) +
